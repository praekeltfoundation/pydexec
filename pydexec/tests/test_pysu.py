...
class TestPysuGosu(object):
    ...
    def test_root_user(self):
        """ Test variations of the user spec for the root user. """
    ...
    def test_unkown_uid_gid(self):
        """
        Test user specs for a UID without a passwd entry or a GID without a
        groups entry.
        """
    ...
    def test_known_user_group(self):
        """ Test user specs for a known (non-root) user and group. """
    ...
    def test_partial_spec(self):
        """ Test empty or group-only user specs. """
        # gosu-t 0: "0:0:$(id -G root)" "root:root:$(id -Gn root)"
        gosu_t('0', '0:0:%s' % (_id('-G', 'root'),), 'root:root:%s' % (_id('-Gn', 'root'),))  # noqa: E501
        # gosu-t '' "$(id -u):$(id -g):$(id -G)" "$(id -un):$(id -gn):$(id -Gn)"  # noqa: E501
